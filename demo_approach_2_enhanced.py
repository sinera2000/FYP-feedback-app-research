...
print("--- ⏳ Initializing Demo for Approach 2 (Enhanced) ---")
load_dotenv()
app = Flask(__name__)
...
# --- ENHANCED APPROACH 2 FEEDBACK LOGIC ---
...
def summarize_strengths_with_groq(user_answer, positive_keywords):
    ...
    try:
        ...
        return f"Your answer was strong when you mentioned: {', '.join(positive_keywords)}"

def explain_missing_concepts_with_groq(ideal_answer, missing_keywords):
    """
    Uses the LLM to generate an explanation of why the missing keywords are important.
    """
    ...
    try:
        ...
        return f"Your answer could be more complete by including key ideas like: **{', '.join(missing_keywords[:5])}**."

def generate_feedback_approach2_enhanced(evaluation, user_answer, ideal_answer):
    """
    An enhanced version of the feedback generator that adds an LLM-powered explanation of missing concepts.
    """
    feedback = ""
    ...
    if not missing_keywords:
         feedback += "\nYour answer is very close! Try to be a bit more detailed."
...
def submit():
    ...
    # Calling the new, enhanced feedback function
    feedback = generate_feedback_approach2_enhanced(evaluation['evaluation'], user_answer, question_doc['Ideal Answer'])
    ...
    return render_template('demo_feedback.html', 
                           ...
                           feedback=feedback,
                           approach_name="Approach 2: LLM Keyword Summary (Enhanced)")
